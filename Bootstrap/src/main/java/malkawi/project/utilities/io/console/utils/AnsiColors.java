package malkawi.project.utilities.io.console.utils;
@SuppressWarnings("unused")
public enum AnsiColors {

    //For Transparency: got the color data below from the internet.
    // Reset
    ...
    public static String format(String s, AnsiColors color) {
        return color.format(s);
    }

    public static String format(String s, AnsiColors... colors) {
        if(colors != null && colors.length > 0) {
            StringBuilder sb = new StringBuilder();
            for (AnsiColors color : colors)
                sb.append(color.code);
            return sb.append(s).append(RESET.code).toString();
        }
        return s;
    }
